...
public class PacketStoreNRun {
    ...
    public static void releaseS2CPackets(MinecraftClient client) {
        ...
        KeyInputHandlerer.interceptedS2CPackets = new ArrayList<>();
    }
    ...
    public static void sendS2CPacket(Packet<?> packet, MinecraftClient client) {
        ...
        try {
            ...
            } else if (packet instanceof BundleCreatedS2CPacket) {
                networkHandler.onBundleCreated((BundleCreatedS2CPacket) packet);
            } else if (packet instanceof NbtQueryPacketS2CPacket) {
                networkHandler.onNbtQueryPacket((NbtQueryPacketS2CPacket) packet);
            } else if (packet instanceof PlayerListS2CPacket) {
                networkHandler.onPlayerList((PlayerListS2CPacket) packet);
            } else if (packet instanceof EntityAttachS2CPacket) { // duplicate check but kept just in case
                networkHandler.onEntityAttach((EntityAttachS2CPacket) packet);
            } else {
                sendMessage("Unknown or unhandled S2C packet: " + packet.getClass().getSimpleName());
            }
        } catch (Exception e) {
            sendMessage("An error occurred while handling S2C packet " + packet.getClass().getSimpleName() + ": " + e);
        }
    }
}
